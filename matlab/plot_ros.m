...
%field tests
plot3(-5,-5,-5,'o','Color','#0072BD','MarkerSize',8);
...
hold off;
 ...
 figure(333);
    ...
    xline(landing(1)*0.01,':','PERFORM_LANDING','FontSize', 8,'Alpha',0.4, 'LabelOrientation', 'aligned','Interpreter','none');
     ...
     title('State Machine Current State');
...
hold off;
 ...
 hold off;
  ...
  mpc_active = find(  states(:,15) == 4 | states(:,15) == 5);
     ...
     title('Shuttle and Target Positions While the MPC is Active');
   ylabel('Position [m]');
    xlabel('Time [s]');
     legend('x_{target}','y_{target}','z_{target}','x_{shuttle}','y_{shuttle}','z_{shuttle}','Interpreter','tex');  
 ...
 hold off;
  ...
  distance =  abs( states(mpc_active(1):mpc_active(end),10) - states(mpc_active(1):mpc_active(end),3));
    ...
    % [min_distance,index] =min(distance);
     ...
     title('Distance Between Drones in the Down Axis While the MPC is Active');
  ...
  distance =  sqrt(( states(mpc_active(1):mpc_active(end),9) - states(mpc_active(1):mpc_active(end),2)).^2 + ( states(mpc_active(1):mpc_active(end),8) - states(mpc_active(1):mpc_active(end),1)).^2);
    ...
    plot((mpc_active(1):mpc_active(end))*0.01, sqrt(( states(mpc_active(1):mpc_active(end),9) - states(mpc_active(1):mpc_active(end),2)).^2 + ( states(mpc_active(1):mpc_active(end),8) - states(mpc_active(1):mpc_active(end),1)).^2), '-');
     hold on;
      ...
      title('Distance Between Drones in the North and East Axis While the MPC is Active');
...
error = ( states(execute_capture(1):execute_capture(end),8) - states(execute_capture(1):execute_capture(end),1)).^2 + ( states(execute_capture(1):execute_capture(end),9) - states(execute_capture(1):execute_capture(end),2)).^2 + ( states(execute_capture(1):execute_capture(end),10) - states(execute_capture(1):execute_capture(end),3)).^2;

MSE = sum(error)/  size(execute_capture,1)
...
error = vecnorm(states(execute_capture(1):execute_capture(end),8:10) - states(execute_capture(1):execute_capture(end),1:3)).^2 ;
